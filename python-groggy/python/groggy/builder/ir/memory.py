...
from .nodes import IRNode, IRDomain
...
class MemoryAnalysis:
    ...
    def _find_in_place_candidates(self):
        ...
        for node in self.ir_graph.nodes:
            ...
            if node.inputs and node.output:
                ...
                if self._can_overwrite_v2(node.id, input_var, output_var, liveness_info):
                    self.in_place_candidates.append(InPlaceCandidate(
                        node=node,
                        input_var=input_var,
    ...
    def _can_overwrite_v2(self, node_id: str, input_var: str, output_var: str,
                          liveness_info: Dict) -> bool:
                ...
                if node.output:
                    for dead_var in newly_dead:
                        if self._buffers_compatible(dead_var, node.output):
                            self.reuse_opportunities.append(BufferReuseOpportunity(
                                dead_var=dead_var,
                                new_var=node.output,
                                node=node,
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "memory_efficiency": (self.peak_memory_bytes / total_allocations * 100) 
    ...
    def print_report(self):
        ...
        if self.in_place_candidates:
            ...
            for i, candidate in enumerate(self.in_place_candidates[:5], 1):
                ...
                print(f"  {i}. {candidate.node.op_type}: {candidate.input_var} → {candidate.output_var}")
...
def analyze_memory(ir_graph: IRGraph, node_count: Optional[int] = None) -> MemoryAnalysis:
    """
