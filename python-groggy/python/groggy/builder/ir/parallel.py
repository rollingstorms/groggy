...
class ParallelExecutionPlan:
    ...
    def to_json(self) -> str:
        ...
        return json.dumps({
            ...
            "sequential_plan": self.sequential_plan.to_json() if self.sequential_plan else None,
...
class ParallelAnalyzer:
    ...
    def _compute_execution_levels(self):
        ...
        in_degree = {node.id: len(self.node_dependencies[node.id]) for node in self.ir_graph.nodes}
        
        # Start with nodes that have no dependencies
        current_level = [node.id for node in self.ir_graph.nodes if in_degree[node.id] == 0]
    ...
    def _estimate_group_parallelism(self, nodes: List[AnyIRNode]) -> float:
        ...
        data_parallel_ops = {
            ...
            "compare", "where", "map", "element_wise"
        }
    ...
    def print_parallelism_report(self):
        ...
        for level_idx, level_nodes in enumerate(self.dependency_levels):
            print(f"Level {level_idx}: {len(level_nodes)} operations (can run in parallel)")
...
def is_thread_safe_op(op_type: str) -> bool:
    ...
    unsafe_ops = {
        "store", "attach", "save",  # Attribute mutations
        "print", "debug",  # I/O operations
    }
