...
from .nodes import AnyIRNode, IRDomain, ControlIRNode
...
class DataflowAnalyzer:
    ...
    def _analyze_loops(self):
        ...
        loop_nodes = [
            ...
            if isinstance(node, ControlIRNode) and node.op_type in ["loop", "until_converged"]
        ]
    ...
    def _try_build_chain(self, start_node: AnyIRNode, 
                        visited: Set[str]) -> Optional[DependencyChain]:
        """
    ...
    def print_analysis(self) -> str:
        ...
        # Liveness summary
        can_drop_count = sum(
            1 for info in self.analysis.liveness.values() 
            if info.can_drop
        )
        inplace_count = sum(
            1 for info in self.analysis.liveness.values()
            if info.can_update_inplace
        )
        ...
        if self.analysis.loops:
            ...
            for loop in self.analysis.loops:
                ...
                if loop.invariant_vars:
                    lines.append(f"    Invariant vars: {', '.join(loop.invariant_vars)}")
