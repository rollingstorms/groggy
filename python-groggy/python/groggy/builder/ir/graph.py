...
from .nodes import IRNode, AnyIRNode, IRDomain
...
class IRGraph:
    ...
    def __init__(self, name: str = "algorithm"):
        ...
        self.var_uses: Dict[str, List[AnyIRNode]] = defaultdict(list)  # var_name -> using nodes
    ...
    def to_dot(self) -> str:
        ...
        for node in self.nodes:
            for input_var in node.inputs:
                def_node = self.get_defining_node(input_var)
                if def_node:
                    lines.append(f'  "{def_node.id}" -> "{node.id}" [label="{input_var}"];')
    ...
    def pretty_print(self) -> str:
        ...
        # Print statistics
        stats = self.stats()
        lines.append(f"Nodes: {stats['total_nodes']}, Variables: {stats['total_variables']}")
        lines.append("")
