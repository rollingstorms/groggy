...
class CoreOps:
    ...
    def compare(self, left: VarHandle, op: str, right: Union[VarHandle, float]) -> VarHandle:
        """
    ...
    def neighbor_agg(self, values: VarHandle, agg: str = "sum", 
                     weights: Optional[VarHandle] = None) -> VarHandle:
        """
    ...
    def collect_neighbor_values(self, values: VarHandle, include_self: bool = True) -> VarHandle:
        """
    ...
    def clip(
        ...
        if min_value is None and max_value is None:
            raise ValueError("core.clip requires at least one of min_value or max_value")
        ...
        for iteration in range(iterations):
            for step in loop_body:
                ...
                if new_step.get("type") == "alias" and "target" in new_step:
                    ...
                    source = new_step.get("source")
                    if target and source:
                        # Map the target name to the (already remapped) source
                        var_mapping[target] = source
...
class BuiltAlgorithm(AlgorithmHandle):
    ...
    def _encode_step(
        ...
        if step_type == "map_nodes":
            ...
            resolved_inputs = {
                key: self._resolve_operand(val, alias_map) for key, val in inputs.items()
            }
    ...
    def _validate(self) -> tuple[list[str], list[str]]:
        ...
        for i, step in enumerate(self._steps):
            ...
            # Exception: update_in_place is allowed to redefine its target (that's its purpose)
            if "output" in step:
                output_var = step["output"]
                if output_var in defined_vars and step_type != "core.update_in_place":
                    warnings.append(
            ...
            if "target" in step:
                ...
                if target_var in defined_vars and step_type != "core.update_in_place":
                    warnings.append(
