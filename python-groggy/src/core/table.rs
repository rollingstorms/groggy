...
impl GroupBy {
    ...
    pub fn agg(&self, ops: HashMap<String, AggregateOp>) -> GraphResult<GraphTable> {
                    ...
                    let agg_result = match op {
                        ...
                        AggregateOp::Mean => {
                            ...
                            if values.is_empty() {
                                ...
                                AttrValue::Float(values.iter().sum::<f64>() / values.len() as f64)
                            }
                        }
        ...
        GraphTable::from_arrays(result_columns, Some(result_column_names))
    }
}
...
impl GraphTable {
    ...
    pub fn from_arrays(arrays: Vec<GraphArray>, column_names: Option<Vec<String>>) -> GraphResult<Self> {
        if arrays.is_empty() {
            return Err(GraphError::InvalidInput {
                message: "Cannot create table from empty array list".to_string(),
                suggestion: "Provide at least one GraphArray".to_string(),
            });
        }
        ...
        for (i, array) in arrays.iter().enumerate() {
            if array.len() != expected_len {
                return Err(GraphError::InvalidInput {
                    message: format!("Array {} has length {} but expected {}", i, array.len(), expected_len),
                    suggestion: "All arrays must have the same length".to_string(),
                });
            }
        }
        ...
        if names.len() != arrays.len() {
            return Err(GraphError::InvalidInput {
                message: format!("Expected {} column names, got {}", arrays.len(), names.len()),
                suggestion: "Provide exactly one name per column".to_string(),
            });
        }
    ...
    pub fn from_graph_nodes(
        ...
        let mut table = Self::from_arrays(columns, Some(column_names))?;
        ...
        if let Some(attr_names) = attrs {
            for &attr_name in attr_names {
                let attr_values: Vec<AttrValue> = edges.iter()
                    .map(|&edge_id| {
                        graph.get_edge_attribute(edge_id, &attr_name.to_string())
    ...
    pub fn set_index(&mut self, index: GraphArray) -> GraphResult<()> {
        if index.len() != self.shape().0 {
            return Err(GraphError::InvalidInput {
                message: format!("Index length {} doesn't match table row count {}", index.len(), self.shape().0),
                suggestion: "Index must have same length as table rows".to_string(),
            });
        }
        ...
        for &col_name in column_names {
            if let Some(array) = self.get_column_by_name(col_name) {
                ...
                return Err(GraphError::InvalidInput {
                    ...
                    suggestion: "Check available column names".to_string(),
                });
            }
        }
        
        let mut table = Self::from_arrays(selected_arrays, Some(selected_names))?;
    ...
    pub fn aggregate(&self, ops: HashMap<String, AggregateOp>) -> GraphResult<HashMap<String, AttrValue>> {
        ...
        for (column_name, op) in ops {
            if let Some(array) = self.get_column_by_name(&column_name) {
                let result = match op {
                    AggregateOp::Sum => array.sum().map(AttrValue::Float).unwrap_or(AttrValue::Int(0)),
                    AggregateOp::Mean => array.mean().map(AttrValue::Float).unwrap_or(AttrValue::Int(0)),
                    AggregateOp::Count => AttrValue::Int(array.count() as i64),
                    AggregateOp::Min => array.min().cloned().unwrap_or(AttrValue::Int(0)),
                    AggregateOp::Max => array.max().cloned().unwrap_or(AttrValue::Int(0)),
                    AggregateOp::Std => array.std().map(AttrValue::Float).unwrap_or(AttrValue::Int(0)),
                    _ => AttrValue::Int(0), // TODO: Implement other operations
                };
    ...
    pub fn describe(&self) -> GraphTable {
        ...
        for stat_name in &stats {
            ...
            for array in &self.columns {
                let value = match stat_name.as_str() {
                    "count" => AttrValue::Int(array.count() as i64),
                    "mean" => array.mean().map(AttrValue::Float).unwrap_or(AttrValue::Int(0)),
                    "std" => array.std().map(AttrValue::Float).unwrap_or(AttrValue::Int(0)),
                    "min" => array.min().cloned().unwrap_or(AttrValue::Int(0)),
                    "max" => array.max().cloned().unwrap_or(AttrValue::Int(0)),
                    _ => AttrValue::Int(0),
                };
    ...
    pub fn to_json(&self) -> GraphResult<String> {
        ...
        serde_json::to_string(&dict).map_err(|e| GraphError::SerializationError {
            message: format!("Failed to serialize table to JSON: {}", e),
            suggestion: "Check that all values are serializable".to_string(),
        })
    }
}
