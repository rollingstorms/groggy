...
impl PyDisplayConfig {
    ...
    pub fn __repr__(&self) -> String {
        format!("DisplayConfig(max_rows={}, max_cols={}, max_width={}, precision={}, use_color={})",
                self.config.max_rows, self.config.max_cols, self.config.max_width,
                self.config.precision, self.config.use_color)
    }
}
...
impl PyTableFormatter {
    ...
    #[staticmethod]
    pub fn format_table(table_data: HashMap<String, PyObject>, config: Option<&PyDisplayConfig>) -> PyResult<String> {
...
fn python_dict_to_rust_hashmap(py_dict: HashMap<String, PyObject>) -> PyResult<HashMap<String, Value>> {
...
pub fn format_table_function(
    ...
    let default_config = groggy::display::DisplayConfig::default();
    let rust_config = config.as_ref().map(|c| c.get_config()).unwrap_or(&default_config);
