...
pub trait TableOps {
    ...
    fn filter(&self, expr: &str) -> PyResult<Self> where Self: Sized;
    ...
    fn join(&self, other: &Self, on: &str) -> PyResult<Self> where Self: Sized;
    
    /// Sort table by column
    fn sort_by(&self, column: &str, ascending: bool) -> PyResult<Self> where Self: Sized;
    
    /// Select specific columns
    fn select(&self, columns: &[&str]) -> PyResult<Self> where Self: Sized;
...
pub trait BaseArrayOps {
    ...
    fn take(&self, n: usize) -> PyResult<Self> where Self: Sized, Self::Item: Clone;
    
    /// Skip first n elements
    fn skip(&self, n: usize) -> PyResult<Self> where Self: Sized, Self::Item: Clone;
}
...
pub trait StatsArrayOps: BaseArrayOps {
    ...
    fn add(&self, other: &Self) -> PyResult<Self> where Self: Sized;
    
    /// Scalar multiplication
    fn multiply(&self, scalar: f64) -> PyResult<Self> where Self: Sized;
