...
use super::traits::{SubgraphOps, TableOps, GraphOps, BaseArrayOps, StatsArrayOps};
...
pub fn forward_array_operations<T: BaseArrayOps>(
    arrays: &[T]
) -> Vec<usize> {
...
impl DynamicDelegation for crate::ffi::subgraphs::subgraph::PySubgraph {
    fn delegate_operation(&self, operation: &str, params: &[&str]) -> PyResult<String> {
        match operation {
            ...
            _ => Err(pyo3::exceptions::PyNotImplementedError::new_err(
                format!("Dynamic operation '{}' not supported", operation)
            )),
        }
    }
}