...
pub fn python_value_to_attr_value(value: &PyAny) -> PyResult<RustAttrValue> {
    ...
    if value.hasattr("__iter__").unwrap_or(false) && !value.is_instance_of::<pyo3::types::PyString>() {
        ...
        if let Ok(list) = value.extract::<Vec<&PyAny>>() {
            ...
            if first_elem.extract::<i64>().is_ok() {
                ...
                let int_str = int_vec.iter().map(|i| i.to_string()).collect::<Vec<_>>().join(",");
            ...
            if first_elem.extract::<f64>().is_ok() || first_elem.extract::<f32>().is_ok() {
                ...
                for item in list {
                    if let Ok(f) = item.extract::<f64>() {
                        ...
                        return Err(PyErr::new::<PyTypeError, _>(
                            format!("Mixed types in list not supported. Expected all floats but found: {}", 
                                item.get_type().name()?)
                        ));
                    }
                }
            ...
            if first_elem.extract::<String>().is_ok() {
                ...
                let strings_json = format!("[{}]", string_vec.iter()
            ...
            return Err(PyErr::new::<PyTypeError, _>(
                format!("Unsupported list element type: {}. Supported list types: [int], [float], [str]", 
