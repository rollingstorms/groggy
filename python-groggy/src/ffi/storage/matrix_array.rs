...
impl PyMatrixArray {
    ...
    fn __getitem__(&self, index: isize) -> PyResult<PyGraphMatrix> {
        ...
        let actual_index = if index < 0 {
            ...
            if positive_index < 0 {
                return Err(PyErr::new::<pyo3::exceptions::PyIndexError, _>(
                    format!("index {} is out of range for array of length {}", index, len)
                ));
            }
        ...
        self.matrices.get(actual_index)
            .cloned()
    ...
    fn dimension_stats(&self) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            let dimensions: Vec<f64> = self.matrices.iter()
                .map(|matrix| {
    ...
    fn correlation_matrix(&self) -> PyResult<Option<f64>> {
        ...
        let dimensions_a: Vec<f64> = self.matrices.iter()
            .map(|matrix| matrix.shape().0 as f64)
            .collect();
            
        let dimensions_b: Vec<f64> = self.matrices.iter()
    ...
    fn stats_summary(&self) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            let sizes: Vec<f64> = self.matrices.iter()
                .map(|matrix| {
    ...
    fn filter_by_size(&self, min_size: usize) -> Self {
        let filtered: Vec<PyGraphMatrix> = self.matrices.iter()
            .filter(|matrix| {
                let (rows, cols) = matrix.shape();
                rows * cols >= min_size
            })
    ...
    fn __str__(&self) -> String {
        if self.matrices.is_empty() {
            ...
            let total_elements: usize = self.matrices.iter()
                .map(|matrix| {
                    let (rows, cols) = matrix.shape();
                    rows * cols
                })
                .sum();
            format!("MatrixArray[{} matrices, {} total elements]", self.matrices.len(), total_elements)
        }
    }
}
