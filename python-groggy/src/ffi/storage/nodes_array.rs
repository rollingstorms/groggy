...
impl PyNodesArray {
    ...
    fn contains(&self, item: &PyNodesAccessor) -> bool {
        ...
        self.base.iter().any(|accessor| accessor.node_count() == target_count)
    }
    ...
    fn filter_by_size(&self, min_size: usize) -> Self {
        let filtered: Vec<PyNodesAccessor> = self.base.iter()
    ...
    fn stats(&self) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            let node_counts: Vec<usize> = self.base.iter()
    ...
    fn __str__(&self) -> String {
        if self.base.is_empty() {
            ...
            let total_nodes: usize = self.base.iter()
                .map(|accessor| accessor.node_count())
                .sum();
