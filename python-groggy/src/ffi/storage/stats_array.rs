//! PyStatsArray - Statistical array operations with numerical methods
//!
//! Wraps the Rust StatsArray to provide mean, sum, std_dev, and other statistical operations
...
#[pyclass(name = "StatsArray", unsendable)]
#[derive(Clone)]
pub struct PyStatsArray {
    pub(crate) inner: StatsArray<f64>,
}

impl PyStatsArray {
    /// Create new PyStatsArray from numerical values
        ...
        for attr in attr_values {
            match attr {
                ...
                _ => return Err(pyo3::exceptions::PyTypeError::new_err(
                    "StatsArray can only contain numerical values (int, float, bool)"
                )),
            }
        }
...
impl PyStatsArray {
    ...
    fn __repr__(&self) -> String {
        ...
        if self.inner.len() <= 5 {
            format!("StatsArray({:?})", preview)
        } else {
            format!("StatsArray({:?}... {} total)", preview, self.inner.len())
        }
    }
    ...
    fn corr(&self, other: &PyStatsArray) -> Option<f64> {
        self.inner.correlate(&other.inner)
    }
    
    /// Element-wise addition with another StatsArray
    fn add(&self, other: &PyStatsArray) -> Option<PyStatsArray> {
        self.inner.add(&other.inner).map(|result| PyStatsArray { inner: result })
    }
    
    /// Multiply all elements by a scalar
    fn multiply(&self, scalar: f64) -> PyStatsArray {
        PyStatsArray {
    ...
    /// Create iterator
    fn __iter__(slf: PyRef<Self>) -> PyStatsArrayIterator {
        PyStatsArrayIterator {
            array: slf.into(),
            index: 0,
        }
    }
...
/// Iterator for PyStatsArray
#[pyclass]
pub struct PyStatsArrayIterator {
    array: Py<PyStatsArray>,
    index: usize,
}

#[pymethods]
impl PyStatsArrayIterator {
