...
impl PyEdgesArray {
    ...
    fn contains(&self, item: &PyEdgesAccessor) -> bool {
        ...
        self.base.iter().any(|accessor| accessor.edge_count() == target_count)
    }
    ...
    fn filter_by_size(&self, min_size: usize) -> Self {
        let filtered: Vec<PyEdgesAccessor> = self.base.iter()
    ...
    fn stats(&self) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            let edge_counts: Vec<usize> = self.base.iter()
    ...
    fn __str__(&self) -> String {
        if self.base.is_empty() {
            ...
            let total_edges: usize = self.base.iter()
                .map(|accessor| accessor.edge_count())
                .sum();
