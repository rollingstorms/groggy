...
def build_pagerank_algorithm(n, damping=0.85, max_iter=20):
    ...
    # Compute out-degrees once (use ranks as nodes reference)
    ...
    with builder.iterate(max_iter):
        ...
        # Handle sink redistribution: collect rank from sinks
        ...
        # Apply damping to neighbor contributions and sink mass
        ...
        # Combine: damping * (neighbor_sum + sink_contrib/n) + (1-damping)/n
        updated = builder.core.add(damped_neighbors, damped_sinks)
        updated = builder.core.add(updated, teleport)
        updated = builder.core.normalize_sum(updated)
...
def benchmark_pagerank(graph, name):
    ...
    # Verify normalization
    total_rank = sum(node.pagerank for node in builder_nodes)
