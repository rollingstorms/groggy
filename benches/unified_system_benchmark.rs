...
use groggy::storage::{
    array::{ArrayOps, BaseArray, LazyArrayIterator, NodesArray},
...
fn generate_test_data(size: usize) -> Vec<AttrValue> {
    (1..=size).map(|i| AttrValue::Int(i as i64)).collect()
}

/// Generate node IDs for testing
fn generate_node_ids(size: usize) -> Vec<NodeId> {
    (1..=size).map(|i| NodeId(i as u32)).collect()
}
...
fn bench_array_creation(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000].iter() {
        ...
        // BaseArray creation
        group.bench_with_input(BenchmarkId::new("BaseArray", size), size, |b, _| {
            let data = generate_test_data(*size);
            b.iter(|| black_box(BaseArray::new(data.clone(), AttrValueType::Int)))
        });

        // Legacy GraphArray creation
        group.bench_with_input(BenchmarkId::new("GraphArray", size), size, |b, _| {
            let data = generate_test_data(*size);
            b.iter(|| black_box(GraphArray::from_vec(data.clone())))
        });
    }
...
fn bench_simple_filter(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("BaseArray_eager", size), size, |b, _| {
        ...
        group.bench_with_input(BenchmarkId::new("BaseArray_lazy", size), size, |b, _| {
            let array = BaseArray::new(data.clone(), AttrValueType::Int);
            b.iter(|| {
                let result = array
                    .lazy_iter()
...
fn bench_node_operations(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("NodesArray_typed", size), size, |b, _| {
        ...
        group.bench_with_input(BenchmarkId::new("Generic_untyped", size), size, |b, _| {
            let generic_array = BaseArray::new(
                node_ids
                    .iter()
                    .map(|id| AttrValue::Int(id.0 as i64))
                    .collect(),
                AttrValueType::Int,
            );
            b.iter(|| {
                // Simulate manual filtering without type safety
                let result: Vec<_> = generic_array
                    .iter()
        ...
        group.bench_with_input(
            ...
            |b, _| {
                ...
                b.iter(|| {
                    ...
                    let step2: Vec<_> = BaseArray::new(step1, AttrValueType::Int)
                        .iter()
