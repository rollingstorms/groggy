...
impl ForceDirectedLayout {
    ...
    fn calculate_attractive_forces(&self, state: &mut SimulationState, edges: &[VizEdge], node_indices: &std::collections::HashMap<String, usize>) {
        for edge in edges {
    ...
    fn apply_constraints(&self, state: &mut SimulationState, node_indices: &std::collections::HashMap<String, usize>) {
...
pub type LayoutFunction = dyn Fn(&[VizNode], &[VizEdge]) -> GraphResult<Vec<(String, Position)>> + Send + Sync;
...
impl CustomLayout {
    ...
    pub fn set_parameter(&mut self, name: &str, value: ParameterValue) -> Result<(), String> {
        if let Some(spec) = self.plugin.parameters.get(name) {
            ...
            if !self.validate_parameter_type(&value, &spec.parameter_type) {
                return Err(format!("Invalid type for parameter '{}': expected {:?}", name, spec.parameter_type));
            }
    ...
    /// Validate parameter type
    fn validate_parameter_type(&self, value: &ParameterValue, expected_type: &ParameterType) -> bool {
    ...
    fn validate_parameter_constraints(&self, value: &ParameterValue, constraints: &ParameterConstraints) -> bool {
        match value {
            ...
            ParameterValue::Integer(i) => {
                if let Some(min) = constraints.min_value {
                    if (*i as f64) < min { return false; }
                }
                if let Some(max) = constraints.max_value {
                    if (*i as f64) > max { return false; }
                }
            }
...
impl LayoutRegistry {
    ...
    where
        F: Fn(&[VizNode], &[VizEdge]) -> GraphResult<Vec<(String, Position)>> + Send + Sync + 'static,
    {
    ...
    pub fn search_by_tags(&self, tags: &[String]) -> Vec<String> {
        ...
        plugins
            .iter()
            .filter(|(_, plugin)| {
                tags.iter().any(|tag| plugin.tags.contains(tag))
            })
    ...
    fn register_builtin_layouts(&self) {
        ...
        let force_directed_plugin = LayoutPlugin {
            ...
            description: "Physics-based force-directed layout with comprehensive simulation".to_string(),
            parameters: {
                let mut params = HashMap::new();
                params.insert("charge".to_string(), ParameterSpec {
                ...
                params.insert("distance".to_string(), ParameterSpec {
                ...
                params.insert("iterations".to_string(), ParameterSpec {
                ...
                params.insert("gravity".to_string(), ParameterSpec {
            ...
            tags: vec!["physics".to_string(), "force-directed".to_string(), "incremental".to_string()],
        };
        ...
        let circular_plugin = LayoutPlugin {
            ...
            parameters: {
                let mut params = HashMap::new();
                params.insert("radius".to_string(), ParameterSpec {
            ...
            author: Some("Groggy Team".to_string()),
            version: Some("1.0.0".to_string()),
            tags: vec!["geometric".to_string(), "circular".to_string(), "simple".to_string()],
        };
        
        // Register as custom layouts
        let force_layout_fn = |nodes: &[VizNode], edges: &[VizEdge]| -> GraphResult<Vec<(String, Position)>> {
            let layout = ForceDirectedLayout::default();
            layout.compute_layout(nodes, edges)
        };
        
        let circular_layout_fn = |nodes: &[VizNode], edges: &[VizEdge]| -> GraphResult<Vec<(String, Position)>> {
...
pub fn create_grid_layout_plugin() -> (LayoutPlugin, Arc<LayoutFunction>) {
    let plugin = LayoutPlugin {
        ...
        parameters: {
            let mut params = HashMap::new();
            params.insert("columns".to_string(), ParameterSpec {
            ...
            params.insert("cell_size".to_string(), ParameterSpec {
        ...
        author: Some("Groggy Team".to_string()),
        version: Some("1.0.0".to_string()),
        tags: vec!["geometric".to_string(), "grid".to_string(), "ordered".to_string()],
    };
    ...
    let layout_fn: Arc<LayoutFunction> = Arc::new(|nodes: &[VizNode], _edges: &[VizEdge]| {
        ...
        let columns = if nodes.len() < 10 { nodes.len() } else { (nodes.len() as f64).sqrt().ceil() as usize };
...
pub fn create_random_layout_plugin() -> (LayoutPlugin, Arc<LayoutFunction>) {
    let plugin = LayoutPlugin {
        ...
        parameters: {
            let mut params = HashMap::new();
            params.insert("width".to_string(), ParameterSpec {
            ...
            params.insert("height".to_string(), ParameterSpec {
    ...
    let layout_fn: Arc<LayoutFunction> = Arc::new(|nodes: &[VizNode], _edges: &[VizEdge]| {
        ...
        for node in nodes {
            ...
            let y = (((seed >> 32) & 0xFFFFFFFF) as f64 / 0xFFFFFFFFu32 as f64) * height - height / 2.0;
...
impl HoneycombLayout {
    ...
    fn energy_optimize(&self, positions: &mut [Position], edges: &[VizEdge], node_indices: &std::collections::HashMap<String, usize>) {
...
impl EnergyBasedLayout {
    ...
    fn initialize_random_positions(&self, nodes: &[VizNode]) -> Vec<Position> {
        ...
        for node in nodes {
            ...
            let angle = ((seed & 0xFFFFFFFF) as f64 / 0xFFFFFFFFu32 as f64) * 2.0 * std::f64::consts::PI;
    ...
    fn calculate_energy(&self, positions: &[Position], edges: &[VizEdge], node_indices: &std::collections::HashMap<String, usize>) -> f64 {
                        ...
                        for j in (i + 1)..positions.len() {
                            ...
                            if distance >= level.range.0 && distance <= level.range.1 {
                                ...
                                if edges.iter().any(|edge| {
                                    (node_indices.get(&edge.source) == Some(&i) && node_indices.get(&edge.target) == Some(&j)) ||
                                    (node_indices.get(&edge.source) == Some(&j) && node_indices.get(&edge.target) == Some(&i))
                                }) {
    ...
    fn simulated_annealing(&self, positions: &mut [Position], edges: &[VizEdge], node_indices: &std::collections::HashMap<String, usize>) {
        ...
        for iteration in 0..self.iterations {
            ...
            if iteration > 100 && (iteration % 100 == 0) {
                let energy_change = (best_energy - current_energy).abs() / best_energy.abs().max(1.0);
...
pub fn register_advanced_layouts() {
    ...
    let honeycomb_plugin = LayoutPlugin {
        ...
        parameters: {
            ...
            params.insert("energy_optimization".to_string(), ParameterSpec {
        ...
        author: Some("Groggy Team".to_string()),
        version: Some("1.0.0".to_string()),
        tags: vec!["geometric".to_string(), "honeycomb".to_string(), "energy".to_string(), "incremental".to_string()],
    };
    
    let honeycomb_fn = |nodes: &[VizNode], edges: &[VizEdge]| -> GraphResult<Vec<(String, Position)>> {
    ...
    let energy_plugin = LayoutPlugin {
        ...
        parameters: {
            ...
            params.insert("cooling_rate".to_string(), ParameterSpec {
            ...
            params.insert("initial_temperature".to_string(), ParameterSpec {
            ...
            params.insert("energy_function".to_string(), ParameterSpec {
        ...
        author: Some("Groggy Team".to_string()),
        version: Some("1.0.0".to_string()),
        tags: vec!["energy".to_string(), "optimization".to_string(), "annealing".to_string(), "incremental".to_string()],
    };
    
    let energy_fn = |nodes: &[VizNode], edges: &[VizEdge]| -> GraphResult<Vec<(String, Position)>> {
