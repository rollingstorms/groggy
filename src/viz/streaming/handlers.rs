...
use super::types::{
    AttributeDisplayType, BoundingBox, CentralityMeasures, EdgeTooltipData, MetricFormat,
    NodeAnalytics, NodeTooltipData, SelectionAnalytics, SelectionType, StreamingError,
    StreamingResult, TooltipAttribute, TooltipMetric, WSMessage,
};
...
impl StreamingServer {
    ...
    pub async fn handle_edge_click_request(&self, edge_id: &str) -> StreamingResult<WSMessage> {
        ...
        let source_node = nodes
            .iter()
            .find(|n| n.id == clicked_edge.source)
            .ok_or_else(|| {
                StreamingError::Client(format!("Source node '{}' not found", clicked_edge.source))
            })?;
        let target_node = nodes
            .iter()
            .find(|n| n.id == clicked_edge.target)
            .ok_or_else(|| {
                StreamingError::Client(format!("Target node '{}' not found", clicked_edge.target))
            })?;
    ...
    pub async fn handle_edge_hover_request(&self, edge_id: &str) -> StreamingResult<WSMessage> {
        ...
        let tooltip_data = EdgeTooltipData {
            title: hovered_edge
                .label
                .clone()
                .unwrap_or_else(|| format!("{} → {}", source_label, target_label)),
    ...
    pub async fn handle_nodes_selection_request(
        &self,
        node_ids: &[String],
        ...
        let total_degree: usize = node_ids
            ...
            .sum();
        let avg_degree = if node_ids.is_empty() {
            0.0
        } else {
            total_degree as f64 / node_ids.len() as f64
        };
