    ...
    fn from(attr_value: &AttrValue) -> Self {
        match attr_value {
            ...
            AttrValue::SubgraphRef(id) => {
                NetworkXValue::String(format!("subgraph_ref:{}", id))
            },
...
pub fn graph_to_networkx(graph: &Graph) -> GraphResult<NetworkXGraph> {
    ...
    for node_id in graph.node_ids() {
        let attributes = graph.get_node_attrs(node_id)?
            .into_iter()
    ...
    for edge_id in graph.edge_ids() {
        ...
        let attributes = graph.get_edge_attrs(edge_id)?
            .into_iter()
            .map(|(k, v)| (k, NetworkXValue::from(&v)))
            .collect();
    ...
    graph_attrs.insert("groggy_version".to_string(), NetworkXValue::String(crate::VERSION.to_string()));
...
pub fn subgraph_to_networkx(subgraph: &Subgraph) -> GraphResult<NetworkXGraph> {
    ...
    for &node_id in subgraph.nodes() {
        let attributes = graph_borrow.get_node_attrs(node_id)?
            .into_iter()
            .map(|(k, v)| (k, NetworkXValue::from(&v)))
            .collect();
    ...
    graph_attrs.insert("groggy_version".to_string(), NetworkXValue::String(crate::VERSION.to_string()));
    graph_attrs.insert("directed".to_string(), NetworkXValue::Boolean(directed));
    graph_attrs.insert("subgraph_type".to_string(), NetworkXValue::String(subgraph.subgraph_type().to_string()));
    graph_attrs.insert("subgraph_node_count".to_string(), NetworkXValue::Integer(subgraph.node_count() as i64));
    graph_attrs.insert("subgraph_edge_count".to_string(), NetworkXValue::Integer(subgraph.edge_count() as i64));
...
pub fn networkx_to_graph(nx_graph: NetworkXGraph) -> GraphResult<Graph> {
    ...
    for nx_edge in &nx_graph.edges {
        let source = node_id_map.get(&nx_edge.source)
            .ok_or_else(|| GraphError::InvalidInput(
                format!("Source node {} not found in node mapping", nx_edge.source)
            ))?;
        let target = node_id_map.get(&nx_edge.target)
            .ok_or_else(|| GraphError::InvalidInput(
                format!("Target node {} not found in node mapping", nx_edge.target)
            ))?;
...
mod tests {
    ...
    fn test_graph_to_networkx() {
        ...
        graph.set_edge_attr(edge, "weight".to_string(), AttrValue::Float(0.8)).unwrap();
    ...
    fn test_subgraph_to_networkx() {
        ...
        graph.set_node_attr(node1, "value".to_string(), AttrValue::Int(1)).unwrap();
        graph.set_node_attr(node2, "value".to_string(), AttrValue::Int(2)).unwrap();
        graph.set_node_attr(node3, "value".to_string(), AttrValue::Int(3)).unwrap();
    ...
    fn test_bidirectional_conversion() {
        ...
        original_graph.set_node_attr(node1, "name".to_string(), AttrValue::Text("A".to_string())).unwrap();
        original_graph.set_node_attr(node2, "name".to_string(), AttrValue::Text("B".to_string())).unwrap();
        
        let edge = original_graph.add_edge(node1, node2).unwrap();
        original_graph.set_edge_attr(edge, "weight".to_string(), AttrValue::Float(1.5)).unwrap();
