...
pub trait GraphEntity: std::fmt::Debug {
    ...
    fn set_attribute(&self, name: AttrName, value: AttrValue) -> GraphResult<()> {
        ...
        match self.entity_id() {
            ...
            EntityId::Edge(id) => {
                graph.pool_mut().set_edge_attribute(id, name, value)
            },
            EntityId::Subgraph(id) | EntityId::Neighborhood(id) |
            EntityId::Component(id) | EntityId::Path(id) |
            EntityId::Filter(id) => {
                graph.pool_mut().set_subgraph_attribute(id, name, value)
            },
        }
    }
    ...
    fn is_active(&self) -> bool {
        ...
        match self.entity_id() {
            ...
            EntityId::Edge(id) => {
                graph.space().is_edge_active(id)
            },
            EntityId::Subgraph(id) | EntityId::Neighborhood(id) |
            EntityId::Component(id) | EntityId::Path(id) |
            EntityId::Filter(id) => {
                graph.space().is_subgraph_active(id)
            },
        }
    }
    ...
    fn contains_entity(&self, other: &dyn GraphEntity) -> bool {
        ...
        if let Ok(related) = self.related_entities() {
            related.iter().any(|entity| entity.entity_id() == other.entity_id())
        } else {
            false
        }
    }
