...
pub trait FilterOperations: SubgraphOperations {
    ...
    fn matches_node_filter(&self, node_id: NodeId) -> GraphResult<bool> {
        self.filter_criteria().matches_node(node_id, &self.graph_ref().borrow())
    }
    ...
    fn matches_edge_filter(&self, edge_id: EdgeId) -> GraphResult<bool> {
        self.filter_criteria().matches_edge(edge_id, &self.graph_ref().borrow())
    }
}
...
pub enum FilterCriteria {
    ...
    NodeAttributeRange { name: AttrName, min: AttrValue, max: AttrValue },
    /// Filter edges by attribute value
    EdgeAttributeEquals { name: AttrName, value: AttrValue },
    /// Filter edges by attribute range
    EdgeAttributeRange { name: AttrName, min: AttrValue, max: AttrValue },
...
impl FilterCriteria {
    ...
    pub fn complexity_score(&self) -> usize {
        match self {
            ...
            FilterCriteria::And(criteria) => criteria.iter().map(|c| c.complexity_score()).sum::<usize>() + 1,
            FilterCriteria::Or(criteria) => criteria.iter().map(|c| c.complexity_score()).sum::<usize>() + 1,
            FilterCriteria::Not(criteria) => criteria.complexity_score() + 1,
        }
    }
    
    /// Check if a node matches this filter criteria
    pub fn matches_node(&self, node_id: NodeId, graph: &crate::api::graph::Graph) -> GraphResult<bool> {
        match self {
            ...
            FilterCriteria::Not(criteria) => {
                Ok(!criteria.matches_node(node_id, graph)?)
            },
    ...
    pub fn matches_edge(&self, edge_id: EdgeId, graph: &crate::api::graph::Graph) -> GraphResult<bool> {
        match self {
            ...
            FilterCriteria::Not(criteria) => {
                Ok(!criteria.matches_edge(edge_id, graph)?)
            },
            _ => Ok(true), // Node criteria always pass for edges (node filtering already done)
        }
    }
