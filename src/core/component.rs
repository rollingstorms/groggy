...
use crate::core::traits::{GraphEntity, SubgraphOperations, ComponentOperations};
...
impl ComponentSubgraph {
    ...
    pub fn from_analysis(
        ...
        let is_largest = all_component_sizes.iter().all(|&size| component_size >= size);
...
impl GraphEntity for ComponentSubgraph {
    ...
    fn summary(&self) -> String {
        let status = if self.is_largest { "largest" } else { "regular" };
        format!(
...
impl ComponentOperations for ComponentSubgraph {
    ...
    fn boundary_nodes(&self) -> GraphResult<Vec<NodeId>> {
        ...
        for &node_id in &self.nodes {
            ...
            let has_external_neighbor = neighbors.iter().any(|&neighbor| !self.nodes.contains(&neighbor));
