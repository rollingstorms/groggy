...
use crate::core::traits::{GraphEntity, SubgraphOperations, FilterOperations};
use crate::core::traits::filter_operations::FilterCriteria;
use crate::types::{EntityId, NodeId, EdgeId};
...
impl SubgraphOperations for FilteredSubgraph {
    ...
    fn connected_components(&self) -> GraphResult<Vec<Box<dyn SubgraphOperations>>> {
        ...
        let result = traversal_engine.connected_components(
            &graph.pool(),
            &mut graph.space(),
            options
        )?;
        ...
        for component in result.components {
            ...
            let filtered_component_edges: HashSet<EdgeId> = component_edges
    ...
    fn bfs(&self, start: NodeId, max_depth: Option<usize>) -> GraphResult<Box<dyn SubgraphOperations>> {
        ...
        let filtered_nodes: HashSet<NodeId> = result.nodes
            .into_iter()
            .filter(|node| self.nodes.contains(node))
            .collect();
        let filtered_edges: HashSet<EdgeId> = result.edges
            .into_iter()
    ...
    fn shortest_path_subgraph(&self, source: NodeId, target: NodeId) -> GraphResult<Option<Box<dyn SubgraphOperations>>> {
        ...
        let x = if let Some(path_result) = traversal_engine.shortest_path(
            ...
            let filtered_nodes: HashSet<NodeId> = path_result.nodes
                .into_iter()
                .filter(|node| self.nodes.contains(node))
                .collect();
            let filtered_edges: HashSet<EdgeId> = path_result.edges
                .into_iter()
                .filter(|edge| self.edges.contains(edge))
                .collect();
...
impl FilterOperations for FilteredSubgraph {
    ...
    fn reapply_filter(&self) -> GraphResult<Box<dyn FilterOperations>> {
        let new_subgraph = FilteredSubgraph::from_criteria(
            self.graph.clone(),
            self.filter_criteria.clone(),
        )?;
    ...
    fn and_filter(&self, other: &dyn FilterOperations) -> GraphResult<Box<dyn FilterOperations>> {
        ...
        let intersected_nodes: HashSet<NodeId> = self.nodes
            .intersection(other.node_set())
            .cloned()
            .collect();
        let intersected_edges: HashSet<EdgeId> = self.edges
            .intersection(other.edge_set())
            .cloned()
            .collect();
    ...
    fn not_filter(&self) -> GraphResult<Box<dyn FilterOperations>> {
        ...
        let complement_nodes: HashSet<NodeId> = all_nodes
            .difference(&self.nodes)
            .cloned()
            .collect();
        let complement_edges: HashSet<EdgeId> = all_edges
    ...
    fn add_criteria(&self, additional_criteria: FilterCriteria) -> GraphResult<Box<dyn FilterOperations>> {
...
mod tests {
    ...
    fn test_filtered_subgraph_creation() {
        ...
        graph.set_node_attr(node1, "type".to_string(), AttrValue::Text("important".to_string())).unwrap();
        graph.set_node_attr(node2, "type".to_string(), AttrValue::Text("normal".to_string())).unwrap();
        graph.set_node_attr(node3, "type".to_string(), AttrValue::Text("important".to_string())).unwrap();
    ...
    fn test_filter_combination() {
        ...
        graph.set_node_attr(node1, "type".to_string(), AttrValue::Text("A".to_string())).unwrap();
        graph.set_node_attr(node1, "value".to_string(), AttrValue::Int(10)).unwrap();
        graph.set_node_attr(node2, "type".to_string(), AttrValue::Text("B".to_string())).unwrap();
        graph.set_node_attr(node2, "value".to_string(), AttrValue::Int(20)).unwrap();
