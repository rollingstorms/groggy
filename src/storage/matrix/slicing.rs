...
use crate::storage::{GraphMatrix, array::{NumArray, SliceIndex, AdvancedIndexing}};
    ...
    fn get_cell(&self, row: usize, col: usize) -> GraphResult<f64> {
        ...
        column.get(row)
            ...
            .ok_or_else(|| GraphError::InvalidInput(
                format!("Cell at [{}, {}] not found", row, col)
            ))
    }
}
...
mod tests {
    ...
    fn test_range_slicing() {
        ...
        let slice = MatrixSlice::new(
            MatrixIndex::Range { start: Some(0), stop: Some(2), step: Some(1) },
            MatrixIndex::Range { start: Some(0), stop: Some(2), step: Some(1) },
        );
