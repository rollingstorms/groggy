...
impl NumArrayBenchmarkResults {
    ...
    pub fn generate_summary(&self) -> String {
        ...
        report.push_str(&format!("**Array sizes tested**: {:?}\n\n", self.config.sizes));
        ...
        for (operation, results) in operations {
            ...
            for result in results {
                report.push_str(&format!(
                    ...
                    result.memory_usage_bytes.map(|m| m.to_string()).unwrap_or_else(|| "N/A".to_string())
                ));
            }
...
impl NumArrayBenchmarker {
    ...
    pub fn run_comprehensive_benchmark(&self) -> GraphResult<NumArrayBenchmarkResults> {
        ...
        println!("Sizes: {:?}, Iterations: {}", self.config.sizes, self.config.iterations);
    ...
    fn benchmark_statistical_operations(&self, size: usize, results: &mut NumArrayBenchmarkResults) -> GraphResult<()> {
        let test_data = self.generate_f64_test_data(size);
        let array = NumArray::new(test_data);
        
        // Benchmark mean calculation
        let mean_result = self.benchmark_operation(
            "mean",
            size,
            || { let _ = array.mean(); }
        );
        ...
        let sum_result = self.benchmark_operation(
            "sum", 
            size,
            || { let _ = array.sum(); }
        );
        ...
        let std_result = self.benchmark_operation(
            "std_dev",
            size, 
            || { let _ = array.std_dev(); }
        );
        ...
        // Benchmark min/max
        let min_result = self.benchmark_operation(
            "min",
            size,
            || { let _ = array.min(); }
        );
        ...
        let max_result = self.benchmark_operation(
            "max",
            size,
            || { let _ = array.max(); }
        );
        ...
        let median_result = self.benchmark_operation(
            "median",
            size,
            || { let _ = array.median(); }
        );
    ...
    fn benchmark_elementwise_operations(&self, size: usize, results: &mut NumArrayBenchmarkResults) -> GraphResult<()> {
        let test_data = self.generate_f64_test_data(size);
        let array = NumArray::new(test_data);
        
        // Benchmark iteration
        let iter_result = self.benchmark_operation(
            "iteration",
            size,
            || { 
                let mut _sum = 0.0;
        ...
        let access_result = self.benchmark_operation(
            "element_access",
            size,
            || {
                let mut _sum = 0.0;
    ...
    fn benchmark_allocation_operations(&self, size: usize, results: &mut NumArrayBenchmarkResults) -> GraphResult<()> {
        let test_data = self.generate_f64_test_data(size);
        
        // Benchmark NumArray creation from Vec
        let create_result = self.benchmark_operation(
            "creation_from_vec",
            size,
            || {
        ...
        let clone_result = self.benchmark_operation(
            "clone",
            size,
            || {
                let _ = array.clone();
    ...
    fn benchmark_iterator_operations(&self, size: usize, results: &mut NumArrayBenchmarkResults) -> GraphResult<()> {
        let test_data = self.generate_f64_test_data(size);
        let array = NumArray::new(test_data);
        
        // Benchmark collect to Vec
        let collect_result = self.benchmark_operation(
            "collect_to_vec",
            size,
            || {
        ...
        // Benchmark filtering 
        let filter_result = self.benchmark_operation(
            "filter",
            size,
            || {
                let _: Vec<&f64> = array.iter()
                    .filter(|&&x| x > 0.5)
                    .collect();
            }
        );
        ...
        // Benchmark mapping
        let map_result = self.benchmark_operation(
            "map",
            size,
            || {
                let _: Vec<f64> = array.iter()
    ...
    fn run_baseline_comparisons(&self, results: &mut NumArrayBenchmarkResults) -> GraphResult<()> {
        ...
        if naive_mean_time.as_nanos() > 0 {
            ...
            results.baseline_comparisons.insert("mean".to_string(), speedup);
        }
        ...
        (0..size)
            .map(|i| {
                ...
                match i % 6 {
                    ...
                    4 => if i % 2 == 0 { i as f64 } else { -(i as f64) }, // Positive/negative
    ...
    pub fn print_results(&self, results: &NumArrayBenchmarkResults) {
        ...
        for (operation, operation_results) in &operations {
            ...
            if let Some(speedup) = results.baseline_improvement(&operation) {
                if speedup > 1.0 {
                    ...
                    println!("  ⚠️  {:.2}x slower than naive implementation", 1.0 / speedup);
                }
            }
        }
        ...
        let slowest_ops: Vec<_> = results.operation_results.iter()
            .filter(|r| r.size == 10_000) // Focus on medium size for analysis
            .collect();
            
        if let Some(slowest) = slowest_ops.iter()
            .max_by_key(|r| r.average_time) {
            println!("⚠️  Slowest operation: {} ({:.2?} for 10K elements)", 
                     slowest.operation, slowest.average_time);
        }
        
        if let Some(fastest) = slowest_ops.iter()
            .min_by_key(|r| r.average_time) {
            println!("⚡ Fastest operation: {} ({:.2?} for 10K elements)", 
                     fastest.operation, fastest.average_time);
        }
        
        // Identify optimization opportunities
        let expensive_ops: Vec<_> = slowest_ops.iter()
        ...
        if !expensive_ops.is_empty() {
            ...
            for op in expensive_ops {
                println!("  • {} could benefit from optimization (currently {:.2?})", 
        ...
        for operation in operations.keys() {
            let op_results: Vec<_> = results.operation_results.iter()
            ...
            if op_results.len() >= 2 {
                ...
                if time_ratio > size_ratio * 1.5 {
                    println!("  • {} shows worse than linear scaling - consider optimization", operation);
...
pub fn benchmark_specific_operation(operation: &str, sizes: Vec<usize>) -> GraphResult<Vec<OperationResult>> {
    let config = NumArrayBenchmarkConfig {
        sizes,
        iterations: 100,
    ...
    Ok(results.operation_results.into_iter()
        .filter(|r| r.operation == operation)
        .collect())
}
