...
impl QueryEvaluator {
    ...
    fn compare_values(&self, left: &AttrValue, right: &AttrValue, operator: &str) -> GraphResult<bool> {
        match (left, right) {
            ...
            (AttrValue::Bool(a), AttrValue::Bool(b)) => {
                Ok(match operator {
                    "==" => a == b,
                    "!=" => a != b,
                    _ => false,
...
mod tests {
    ...
    fn test_length_expressions() {
        ...
        assert!(evaluator.evaluate(&AttrValue::Text("hello world".to_string())).unwrap());
        assert!(!evaluator.evaluate(&AttrValue::Text("hi".to_string())).unwrap());
    }
