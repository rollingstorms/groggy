...
use crate::storage::array::{ArrayOps, ArrayIterator, LazyArrayIterator};
...
impl Benchmarker {
    ...
    pub fn run_comprehensive_benchmark(&self) -> GraphResult<BenchmarkResults> {
        println!("Running comprehensive benchmark with {} elements, {} iterations...", 
    ...
    pub fn print_results(&self, results: &BenchmarkResults) {
        ...
        if results.lazy_is_faster() {
            println!("  ✅ Lazy is {:.1}% FASTER", results.performance_improvement());
        } else {
            println!("  ❌ Lazy is {:.1}% SLOWER", -results.performance_improvement());
        }
        ...
        if let Some(ref memory) = results.memory_usage {
            ...
            let memory_savings = (memory.eager_peak - memory.lazy_peak) as f64 / memory.eager_peak as f64 * 100.0;
