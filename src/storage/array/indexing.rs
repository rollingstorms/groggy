            ...
            SliceIndex::BoolArray(mask) => {
                if mask.len() != length {
                    return Err(GraphError::InvalidInput(
                        format!("Boolean mask length {} doesn't match array length {}", 
                               mask.len(), length)
                    ));
                }
                Ok(mask.nonzero())
...
fn resolve_negative_index(index: i64, length: usize) -> GraphResult<usize> {
    ...
    if resolved < 0 || resolved >= len {
        return Err(GraphError::InvalidInput(
            format!("Index {} out of range for array of length {}", index, length)
        ));
    }
...
fn resolve_range_indices(
    ...
    if step == 0 {
        return Err(GraphError::InvalidInput("Slice step cannot be zero".to_string()));
    }
...
mod tests {
    ...
    fn test_range_index_resolution() {
        // Test simple range
        let idx = SliceIndex::Range { start: Some(1), stop: Some(4), step: Some(1) };
        let indices = idx.resolve_indices(10).unwrap();
        assert_eq!(indices, vec![1, 2, 3]);
        
        // Test step size
        let idx = SliceIndex::Range { start: Some(0), stop: Some(10), step: Some(2) };
        let indices = idx.resolve_indices(10).unwrap();
        assert_eq!(indices, vec![0, 2, 4, 6, 8]);
        
        // Test None defaults
        let idx = SliceIndex::Range { start: None, stop: Some(3), step: None };
        let indices = idx.resolve_indices(10).unwrap();
    ...
    fn test_advanced_indexing_base_array() {
        ...
        // Test range indexing
        let range_idx = SliceIndex::Range { start: Some(1), stop: Some(4), step: Some(1) };
