    ...
    Send + Sync + 'static
{
    ...
    fn simd_fma(a: &[Self], b: &[Self], c: &[Self], result: &mut [Self]) {
        for (((a_val, b_val), c_val), result_val) in a.iter().zip(b.iter()).zip(c.iter()).zip(result.iter_mut()) {
...
impl NumericType for f64 {
    ...
    fn from_f64(val: f64) -> Option<Self> { Some(val) }
...
impl NumericType for i64 {
    ...
    fn is_finite(self) -> bool { true }  // Integers are always finite
    fn abs(self) -> Self { self.abs() }
...
impl NumericType for bool {
    ...
    fn to_f64(self) -> f64 { if self { 1.0 } else { 0.0 } }
    
    fn is_finite(self) -> bool { true }
    fn abs(self) -> Self { self }  // abs(bool) = bool
    
    fn add(self, other: Self) -> Self { self | other }  // Logical OR
    fn sub(self, other: Self) -> Self { self & !other } // Logical NAND
    fn mul(self, other: Self) -> Self { self & other }  // Logical AND
    fn div(self, other: Self) -> Self { 
        if other { self } else { false }  // Division by false is false
    }
...
impl NumericType for i128 {
    ...
    fn is_finite(self) -> bool { true }  // Integers are always finite
    fn abs(self) -> Self { self.saturating_abs() }  // Use saturating to avoid overflow on i128::MIN
