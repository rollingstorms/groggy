...
impl<T: NumericType> MatrixOperations<T> for UnifiedMatrix<T> {
        ...
        F: Fn(&T, &T) -> T + Send + Sync
    {
        ...
        let result_vec: Vec<T> = self_vec.iter()
            .zip(other_vec.iter())
            .map(|(a, b)| f(a, b))
            .collect();
    ...
    fn reshape(&self, new_rows: usize, new_cols: usize) -> MatrixResult<UnifiedMatrix<T>> {
        if new_rows * new_cols != self.len() {
            return Err(MatrixError::InvalidShape(
                format!("Cannot reshape {}x{} matrix to {}x{}", 
    ...
    fn slice(
        ...
        if row_end > self.rows() || col_end > self.cols() || row_start >= row_end || col_start >= col_end {
            return Err(MatrixError::InvalidShape(
                format!("Invalid slice indices: rows {}..{}, cols {}..{} for shape {:?}",
                    row_start, row_end, col_start, col_end, self.shape().as_tuple())
            ));
        }
...
impl MathFunctions {
    ...
    pub fn exp<T: NumericType>(matrix: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        matrix.map_elementwise(|x| {
            T::from_f64(x.to_f64().exp()).unwrap_or(T::zero())
        })
    }
    ...
    /// Element-wise power
    pub fn pow<T: NumericType>(matrix: &UnifiedMatrix<T>, exponent: f64) -> MatrixResult<UnifiedMatrix<T>> {
        matrix.map_elementwise(|x| {
            T::from_f64(x.to_f64().powf(exponent)).unwrap_or(T::zero())
        })
    }
...
impl MatrixUtils {
    ...
    pub fn constant<T: NumericType>(rows: usize, cols: usize, value: T) -> MatrixResult<UnifiedMatrix<T>> {
    ...
    pub fn from_fn<T: NumericType, F>(rows: usize, cols: usize, f: F) -> MatrixResult<UnifiedMatrix<T>>
    where
        F: Fn(usize, usize) -> T
    {
        let mut data = Vec::with_capacity(rows * cols);
