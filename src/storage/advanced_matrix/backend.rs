...
use crate::storage::advanced_matrix::numeric_type::{NumericType, DType};
use std::fmt::Debug;
...
pub enum BackendError {
    ...
    IncompatibleDimensions { expected: (usize, usize), got: (usize, usize) },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            BackendError::IncompatibleDimensions { expected, got } => {
                write!(f, "Incompatible dimensions: expected {:?}, got {:?}", expected, got)
            }
...
pub trait ComputeBackend: Send + Sync + Debug {
    ...
    unsafe fn gemm_raw(
        &self,
        dtype: DType,
        a: *const u8, a_shape: (usize, usize),
        b: *const u8, b_shape: (usize, usize),
        c: *mut u8, c_shape: (usize, usize),
    ...
    unsafe fn gemv_raw(
        &self,
        dtype: DType,
        matrix: *const u8, matrix_shape: (usize, usize),
        vector: *const u8,
        result: *mut u8,
        alpha: *const u8, beta: *const u8,
    ) -> BackendResult<()>;
    ...
    unsafe fn elementwise_add_raw(
        &self,
        dtype: DType,
        a: *const u8, b: *const u8, result: *mut u8, len: usize,
    ) -> BackendResult<()>;
    ...
    unsafe fn reduce_sum_raw(&self, dtype: DType, data: *const u8, len: usize, result: *mut u8) -> BackendResult<()>;
    unsafe fn reduce_max_raw(&self, dtype: DType, data: *const u8, len: usize, result: *mut u8) -> BackendResult<()>;
    unsafe fn reduce_min_raw(&self, dtype: DType, data: *const u8, len: usize, result: *mut u8) -> BackendResult<()>;
}
...
pub trait ComputeBackendExt: ComputeBackend {
    ...
    fn gemm<T: NumericType>(
        &self,
        a: &[T], a_shape: (usize, usize),
        b: &[T], b_shape: (usize, usize),
        c: &mut [T], c_shape: (usize, usize),
        alpha: T, beta: T,
    ) -> BackendResult<()> {
        unsafe {
            self.gemm_raw(
    ...
    fn reduce_max<T: NumericType>(&self, data: &[T]) -> BackendResult<T> {
        if data.is_empty() {
            return Err(BackendError::InvalidInput("Empty data for reduction".to_string()));
        }
...
impl ComputeBackend for NativeBackend {
    ...
    fn supports_dtype(&self, _dtype: DType) -> bool { true }
    ...
    fn optimal_threshold(&self, _op: OperationType) -> usize { 0 }
    ...
    unsafe fn gemm_raw(
        ...
        match dtype {
            DType::Float64 => self.gemm_typed::<f64>(a, a_shape, b, b_shape, c, c_shape, alpha, beta),
            DType::Float32 => self.gemm_typed::<f32>(a, a_shape, b, b_shape, c, c_shape, alpha, beta),
    ...
    unsafe fn gemv_raw(
        ...
        match dtype {
            DType::Float64 => self.gemv_typed::<f64>(matrix, matrix_shape, vector, result, alpha, beta),
            DType::Float32 => self.gemv_typed::<f32>(matrix, matrix_shape, vector, result, alpha, beta),
            DType::Int64 => self.gemv_typed::<i64>(matrix, matrix_shape, vector, result, alpha, beta),
            DType::Int32 => self.gemv_typed::<i32>(matrix, matrix_shape, vector, result, alpha, beta),
            DType::Bool => self.gemv_typed::<bool>(matrix, matrix_shape, vector, result, alpha, beta),
            _ => Err(BackendError::UnsupportedType(dtype)),
        }
    }
...
impl NativeBackend {
    ...
    unsafe fn reduce_sum_typed<T: NumericType>(
        &self,
        data: *const u8, len: usize, result: *mut u8,
    ) -> BackendResult<()> {
...
impl BackendSelector {
    ...
    pub fn select_backend(
        ...
        let candidates: Vec<_> = self.backends
            .iter()
            .filter(|backend| {
        ...
        let best_backend = match hint {
            BackendHint::PreferSpeed => {
                self.select_fastest_backend(&candidates, op, size)
            }
            BackendHint::PreferMemory => {
                self.select_memory_efficient_backend(&candidates, op, size)
            }
            BackendHint::PreferGPU => {
                self.select_gpu_backend(&candidates).unwrap_or_else(|| candidates[0])
            }
            BackendHint::PreferCPU => {
                self.select_cpu_backend(&candidates).unwrap_or_else(|| candidates[0])
            }
            BackendHint::AutoSelect => {
                self.auto_select_backend(&candidates, op, size, dtype)
            }
    ...
    fn select_memory_efficient_backend<'a>(
        ...
        candidates
            .iter()
            .max_by(|a, b| {
                let mem_a = a.performance_characteristics(op).memory_bandwidth_gb_per_sec;
                let mem_b = b.performance_characteristics(op).memory_bandwidth_gb_per_sec;
        ...
        for backend in candidates {
            ...
            match backend.name() {
                name if name.contains("MKL") => {
                    ...
                    if size > 1000 { score += 40.0; }
                }
                "OpenBLAS" => {
                    // OpenBLAS gets bonus for matrix operations
                    if matches!(op, OperationType::GEMM | OperationType::GEMV) { score += 30.0; }
                }
                "NumPy" => {
                    // NumPy gets bonus for decompositions and medium-sized operations
                    if matches!(op, OperationType::SVD | OperationType::QR | OperationType::Cholesky) { score += 35.0; }
                    if size > 100 && size < 5000 { score += 20.0; }
                }
                "Native" => {
                    // Native backend gets bonus for small operations and always-available
                    if size < 100 { score += 25.0; }
