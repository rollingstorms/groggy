...
pub trait ActivationFunction<T: NumericType> {
    ...
    fn backward(&self, input: &UnifiedMatrix<T>, grad_output: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>>;
...
impl ActivationOps {
    ...
    pub fn relu_inplace<T: NumericType>(matrix: &mut UnifiedMatrix<T>) -> MatrixResult<()> {
        Self::apply_elementwise_inplace(matrix, |x| {
            if x.to_f64() > 0.0 {
                *x
            } else {
                T::zero()
    ...
    pub fn relu_derivative<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        Self::apply_elementwise(input, |x| {
            if x.to_f64() > 0.0 {
                T::one()
            } else {
    ...
    pub fn gelu_derivative<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        Self::apply_elementwise(input, |x| {
            let x_f64 = x.to_f64();
            let sqrt_2_over_pi = (2.0 / PI).sqrt();
            ...
            let cdf_approx = 0.5 * (1.0 + (sqrt_2_over_pi * (x_f64 + 0.044715 * x_f64.powi(3))).tanh());
            let pdf_part = 0.5 * x_f64 * gaussian * sqrt_2_over_pi * (1.0 + 3.0 * 0.044715 * x_f64 * x_f64);
    ...
    pub fn sigmoid_derivative<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
    ...
    pub fn tanh_derivative<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        Self::apply_elementwise(input, |x| {
    ...
    pub fn swish_derivative<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        let sigmoid_out = Self::sigmoid(input)?;
    ...
    pub fn leaky_relu<T: NumericType>(input: &UnifiedMatrix<T>, alpha: f64) -> MatrixResult<UnifiedMatrix<T>> {
        Self::apply_elementwise(input, |x| {
            let x_f64 = x.to_f64();
    ...
    pub fn elu<T: NumericType>(input: &UnifiedMatrix<T>, alpha: f64) -> MatrixResult<UnifiedMatrix<T>> {
        Self::apply_elementwise(input, |x| {
            let x_f64 = x.to_f64();
            let result = if x_f64 > 0.0 { 
                x_f64 
    ...
    pub fn softmax<T: NumericType>(input: &UnifiedMatrix<T>) -> MatrixResult<UnifiedMatrix<T>> {
        ...
        for row in 0..shape.rows {
            ...
            for (col, &exp_val) in exp_values.iter().enumerate() {
                ...
                Self::set_element(&mut result, row, col, T::from_f64(softmax_val).unwrap_or(T::zero()))?;
            }
        }
...
pub fn relu<T: NumericType>() -> ReLU { ReLU }
pub fn gelu<T: NumericType>() -> GELU { GELU }
pub fn sigmoid<T: NumericType>() -> Sigmoid { Sigmoid }
pub fn tanh<T: NumericType>() -> Tanh { Tanh }
