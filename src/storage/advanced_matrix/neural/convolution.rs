...
impl ConvolutionConfig {
    ...
    pub fn output_dimensions(&self, input_height: usize, input_width: usize) -> (usize, usize) {
        ...
        let (pad_h, pad_w) = match self.padding {
            ...
            PaddingMode::Custom { pad_height, pad_width } => (pad_height, pad_width),
        };
...
impl<T: NumericType> Conv2D<T> {
    ...
    fn forward_with_im2col(&mut self, input: &ConvTensor<T>) -> MatrixResult<ConvTensor<T>> {
        ...
        let weight_in_channels = self.weights.shape().cols / (self.config.kernel_size.0 * self.config.kernel_size.1);
                ...
                for channel in 0..in_channels {
                    for kernel_row in 0..kernel_h {
                        for kernel_col in 0..kernel_w {
                            ...
                            let padded_row = if input_row >= pad_h { input_row - pad_h } else { usize::MAX };
                            let padded_col = if input_col >= pad_w { input_col - pad_w } else { usize::MAX };
                            
                            // Get value (zero if outside bounds)
                            let value = if padded_row < input_h && padded_col < input_w && 
                                          padded_row != usize::MAX && padded_col != usize::MAX {
