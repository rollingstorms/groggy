...
use crate::storage::advanced_matrix::{
    ...
    backend::{ComputeBackend, BackendError, BackendResult, OperationType, BackendPerformance},
};
...
impl ComputeBackend for NumPyBackend {
    ...
    fn performance_characteristics(&self, op: OperationType) -> BackendPerformance {
        ...
        match op {
            ...
            OperationType::SVD | OperationType::QR | OperationType::Cholesky => BackendPerformance {
    ...
    unsafe fn gemm_raw(
        &self,
        _dtype: DType,
        _a: *const u8, _a_shape: (usize, usize),
        _b: *const u8, _b_shape: (usize, usize),
        _c: *mut u8, _c_shape: (usize, usize),
        _alpha: *const u8, _beta: *const u8,
    ) -> BackendResult<()> {
        Err(BackendError::BackendUnavailable("NumPy backend not implemented (stub)".to_string()))
    }
    
    unsafe fn gemv_raw(
        &self,
        _dtype: DType,
        _matrix: *const u8, _matrix_shape: (usize, usize),
        _vector: *const u8,
        _result: *mut u8,
        _alpha: *const u8, _beta: *const u8,
    ) -> BackendResult<()> {
    ...
    unsafe fn elementwise_add_raw(
        &self,
        _dtype: DType,
        _a: *const u8, _b: *const u8, _result: *mut u8, _len: usize,
    ) -> BackendResult<()> {
        Err(BackendError::BackendUnavailable("NumPy backend not implemented (stub)".to_string()))
    }
    ...
    unsafe fn reduce_sum_raw(&self, _dtype: DType, _data: *const u8, _len: usize, _result: *mut u8) -> BackendResult<()> {
        Err(BackendError::BackendUnavailable("NumPy backend not implemented (stub)".to_string()))
    }
    
    unsafe fn reduce_max_raw(&self, _dtype: DType, _data: *const u8, _len: usize, _result: *mut u8) -> BackendResult<()> {
        Err(BackendError::BackendUnavailable("NumPy backend not implemented (stub)".to_string()))
    }
    
    unsafe fn reduce_min_raw(&self, _dtype: DType, _data: *const u8, _len: usize, _result: *mut u8) -> BackendResult<()> {
        Err(BackendError::BackendUnavailable("NumPy backend not implemented (stub)".to_string()))
    }
}
