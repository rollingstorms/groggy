...
use crate::storage::advanced_matrix::{
    ...
    backend::{BackendHint, BackendSelector, OperationType, ComputeBackendExt},
    memory::{SharedBuffer, MatrixLayout, MemoryError},
};
...
pub enum MatrixError {
    ...
    InvalidIndex { row: usize, col: usize, shape: (usize, usize) },
}
...
impl<T: NumericType> UnifiedMatrix<T> {
    ...
    pub fn get(&self, row: usize, col: usize) -> MatrixResult<T> {
        if row >= self.shape.rows || col >= self.shape.cols {
            return Err(MatrixError::InvalidIndex { 
                row, col, shape: self.shape.as_tuple() 
            });
        }
        ...
        match &self.storage {
            ...
            MatrixStorage::Sparse(_) => {
                Err(MatrixError::UnsupportedOperation("Sparse matrix indexing not implemented".to_string()))
            }
            MatrixStorage::View(_) => {
                Err(MatrixError::UnsupportedOperation("Matrix view indexing not implemented".to_string()))
            }
            MatrixStorage::Lazy(_) => {
                Err(MatrixError::UnsupportedOperation("Lazy matrix indexing not implemented".to_string()))
            }
        }
    }
    ...
    pub fn set(&mut self, row: usize, col: usize, value: T) -> MatrixResult<()> {
        ...
        match &mut self.storage {
            ...
            _ => Err(MatrixError::UnsupportedOperation("Setting elements in non-dense matrices not implemented".to_string()))
        }
    }
    ...
    pub fn fill(&mut self, value: T) -> MatrixResult<()> {
        match &self.storage {
            ...
            _ => Err(MatrixError::UnsupportedOperation("Filling non-dense matrices not implemented".to_string()))
        }
    }
    ...
    pub fn matmul(&self, other: &Self) -> MatrixResult<Self> {
        ...
        match (&self.storage, &other.storage, &result.storage) {
            (MatrixStorage::Dense(a_buf), MatrixStorage::Dense(b_buf), MatrixStorage::Dense(c_buf)) => {
                let a_data = a_buf.data()?;
                let b_data = b_buf.data()?;
                let mut c_data = c_buf.data_mut()?;
                
                backend.gemm(
                    a_data.as_slice(), self.shape.as_tuple(),
                    b_data.as_slice(), other.shape.as_tuple(),
                    c_data.as_slice_mut(), result.shape.as_tuple(),
                    T::one(),  // alpha
                    T::zero(), // beta
    ...
    pub fn mul(&self, other: &Self) -> MatrixResult<Self> {
        ...
        match (&self.storage, &other.storage, &mut result.storage) {
            ...
            _ => {
                return Err(MatrixError::UnsupportedOperation(
                    "Element-wise multiplication for non-dense matrices not implemented".to_string()
                ));
            }
        }
    ...
    pub fn sum(&self) -> MatrixResult<T::Accumulator> {
        ...
        match &self.storage {
            MatrixStorage::Dense(buffer) => {
                let data = buffer.data()?;
                backend.reduce_sum(data.as_slice())
                    .map_err(|e| MatrixError::ComputationError(e.to_string()))
            }
    ...
    pub fn max(&self) -> MatrixResult<T> {
        ...
        match &self.storage {
            MatrixStorage::Dense(buffer) => {
                let data = buffer.data()?;
                backend.reduce_max(data.as_slice())
                    .map_err(|e| MatrixError::ComputationError(e.to_string()))
            }
            _ => Err(MatrixError::UnsupportedOperation(
    ...
    pub fn min(&self) -> MatrixResult<T> {
        ...
        match &self.storage {
            MatrixStorage::Dense(buffer) => {
                let data = buffer.data()?;
                backend.reduce_min(data.as_slice())
                    .map_err(|e| MatrixError::ComputationError(e.to_string()))
            }
            _ => Err(MatrixError::UnsupportedOperation(
    ...
    pub fn to_vec(&self) -> MatrixResult<Vec<T>> {
        match &self.storage {
            MatrixStorage::Dense(buffer) => {
                buffer.to_vec().map_err(MatrixError::from)
            }
            _ => Err(MatrixError::UnsupportedOperation(
