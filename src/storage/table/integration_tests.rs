...
mod tests {
    ...
    fn test_table_column_iter_chaining() {
        // Create test data
        let ages = BaseArray::with_name(
            vec![AttrValue::Int(25), AttrValue::Int(30), AttrValue::Int(35), AttrValue::Int(20)],
            AttrValueType::Int,
            "age".to_string()
        );
    ...
    fn test_table_operations() {
        ...
        let salaries = BaseArray::with_name(
            vec![AttrValue::Float(50000.0), AttrValue::Float(60000.0), AttrValue::Float(70000.0)],
            AttrValueType::Float,
            "salary".to_string()
        );
    ...
    fn test_column_access_properties() {
        ...
        let active = BaseArray::with_name(
            vec![AttrValue::Bool(true), AttrValue::Bool(false), AttrValue::Bool(true)],
            AttrValueType::Bool,
            "active".to_string()
        );
        ...
        let second_column = table.column_by_index(1).expect("Second column should exist");
        assert!(table.column_by_index(2).is_none()); // Should not exist
        
        // Verify we have both columns
        assert!(first_column.dtype() == &AttrValueType::Int || first_column.dtype() == &AttrValueType::Bool);
        assert!(second_column.dtype() == &AttrValueType::Int || second_column.dtype() == &AttrValueType::Bool);
