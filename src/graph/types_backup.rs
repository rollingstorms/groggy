...
/// Lightweight node data that only stores ID and attribute UIDs (CURRENT/PREFERRED)
...
/// Lightweight edge data that only stores source/target and attribute UIDs (CURRENT/PREFERRED)
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Graph type enum to support both directed and undirected graphs with hybrid storage (CURRENT/PREFERRED)
#[derive(Debug, Clone)]
