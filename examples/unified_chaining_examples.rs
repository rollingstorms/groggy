...
use groggy::prelude::*;
use groggy::storage::array::{BaseArray, NodesArray, EdgesArray, ArrayOps};
use groggy::types::{AttrValue, AttrValueType, NodeId, EdgeId};
...
pub fn example_basic_eager_operations() -> GraphResult<()> {
    ...
    let adults: Vec<AttrValue> = ages.iter()
    ...
    let doubled_ages: Vec<AttrValue> = ages.iter()
...
pub fn example_lazy_evaluation() -> GraphResult<()> {
    ...
    println!("Lazy result (5 samples from 500-800 range): {} elements", result.len());
    
    // Show the optimization in action
    let optimized_result = large_array.lazy_iter()
        .take(100)                    // Early termination - only process first 100
        .filter("value > 50")         // Filter on reduced dataset
        .collect()?;
    
    println!("Early termination result: {} elements", optimized_result.len());
...
pub fn example_node_operations() -> GraphResult<()> {
    ...
    let neighbor_lists = nodes.lazy_iter()
...
pub fn example_edge_operations() -> GraphResult<()> {
    ...
    let specific_edges = edges.lazy_iter()
        .filter_by_endpoints(
...
pub fn example_performance_comparison() -> GraphResult<()> {
    ...
    let data: Vec<AttrValue> = (1..=50000)
        .map(|i| AttrValue::Int(i))
        .collect();
    ...
    let eager_result: Vec<_> = array.iter()
    ...
    let start = Instant::now();
    let lazy_result = array.lazy_iter()
    ...
    println!("Eager result: {} items in {:?}", eager_result.len(), eager_time);
    println!("Lazy result: {} items in {:?}", lazy_result.len(), lazy_time);
...
pub fn example_error_handling() -> GraphResult<()> {
    ...
    let result = mixed_array.lazy_iter()
...
pub fn example_custom_benchmarking() -> GraphResult<()> {
    ...
    println!("Running benchmarks with {} data sizes and {} operation chains...", 
...
pub fn example_table_integration() -> GraphResult<()> {
    ...
    let high_degree_nodes = degree_array.lazy_iter()
        .filter("value > 2")
        .collect()?;
